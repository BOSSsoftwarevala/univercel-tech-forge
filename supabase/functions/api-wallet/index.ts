...
serve(async (req: Request) => {
  ...
  const path = url.pathname
    .replace(/^.*\/functions\/v1\/api-wallet/, "")
    .replace(/^.*\/api-wallet/, "") || "/";
  ...
  // POST /wallet/topup
  if (path === "/topup" && req.method === "POST") {
    return withAuth(req, [], async ({ supabaseAdmin, body, user }) => {
      const validation = validateRequired(body, ["amount", "payment_method"]);
      if (validation) return errorResponse(validation);
      ...
      const transactionRef = body.transaction_ref || `TOPUP_${user.userId}_${Date.now()}`;
      ...
      if (!wallet) {
        ...
        if (createWalletError || !newWallet) {
          return errorResponse(createWalletError?.message || "Unable to create wallet", 500);
        }
      ...
      if (updateError) {
        return errorResponse("Topup failed - please retry", 409);
      }
      ...
      const { data: transaction, error: transactionError } = await supabaseAdmin
        ...
        .select("transaction_id, amount, status, timestamp, reference")
        .single();
      ...
      if (transactionError) {
        await supabaseAdmin
          ...
          .eq("wallet_id", wallet.wallet_id);
        return errorResponse(transactionError.message, 500);
      }

      await logWalletAudit(supabaseAdmin, user.userId, "topup", "completed", {
        amount,
        previous_balance: currentBalance,
        new_balance: newBalance,
        wallet_id: wallet.wallet_id,
      ...
      await createAuditLog(supabaseAdmin, user.userId, user.role, "wallet", "topup_completed", {
        amount,
        payment_method: body.payment_method,
        transaction_ref: transactionRef,
        new_balance: newBalance,
    ...
    }, { module: "wallet", action: "topup", rateLimitType: "payment" });
  }
  ...
  if (path.startsWith("/invoice/") && req.method === "GET") {
    return withAuth(req, [], async ({ supabaseAdmin, user }) => {
      ...
      const { data: transaction, error } = await supabaseAdmin
        .from("transactions")
        .select("transaction_id, type, amount, reference, status, timestamp")
      ...
      if (wallet?.wallet_id) {
        const { data: ownedTransaction } = await supabaseAdmin
          .from("transactions")
        ...
        if (!ownedTransaction && !["boss_owner", "admin", "finance_manager", "super_admin"].includes(user.role)) {
          return errorResponse("Invoice not found", 404);
        }
      }
      ...
      return jsonResponse({
        ...
        issued_at: transaction.timestamp,
      });
    }, { module: "wallet", action: "invoice" });
  }
