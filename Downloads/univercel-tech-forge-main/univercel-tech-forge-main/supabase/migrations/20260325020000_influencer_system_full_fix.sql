...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'unique_influencer_reference_key'
        AND table_name = 'influencer_commission_journal'
    ) THEN
        ALTER TABLE public.influencer_commission_journal
        ADD CONSTRAINT unique_influencer_reference_key UNIQUE (influencer_id, reference_key);
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Admins manage fraud flags" ON public.influencer_fraud_flags;
...
DROP POLICY IF EXISTS "Admins manage AI contents" ON public.influencer_ai_contents;
CREATE POLICY "Admins manage AI contents" ON public.influencer_ai_contents FOR ALL USING (can_manage_influencers(auth.uid()));

DROP POLICY IF EXISTS "Influencers manage own AI contents" ON public.influencer_ai_contents;
...
DROP POLICY IF EXISTS "Admins and finance manage approval logs" ON public.influencer_approval_logs;
...
DROP POLICY IF EXISTS "Admins view trace logs" ON public.influencer_trace_logs;
...
DROP POLICY IF EXISTS "Admins manage commission rules" ON public.influencer_commission_rules;
...
DROP POLICY IF EXISTS "Finance view commission rules" ON public.influencer_commission_rules;
