...
serve(async (req) => {
  ...
  try {
    ...
    const { action, serverId, serverData, submissionId, autoTrigger = false } = await req.json();

    // Auto-trigger analysis for new server submissions
    if (autoTrigger && action === 'auto_analyze') {
      return await handleAutoAnalysis(supabase, serverData);
    }
...
async function handleAutoAnalysis(supabase: any, serverData: any) {
  try {
    // Perform comprehensive AI analysis
    const preCheck = await preCheckSubmission(serverData);

    if (preCheck.approved) {
      // Auto-approve and trigger actions
      ...
      if (preCheck.riskLevel === 'low') {
        analysisResult.actions.push('security_scan_scheduled');
        // Schedule security scan
        await supabase.from('server_analysis_queue').insert({
      ...
      analysisResult.actions.push('compliance_check_scheduled');
      await supabase.from('server_analysis_queue').insert({
        server_data: serverData,
      ...
      // Send notification to global monitoring
      await supabase.from('global_monitor_events').insert({
        event_type: 'server_auto_analyzed',
        data: {
          serverData,
          analysisResult,
      ...
      return new Response(JSON.stringify({
        success: true,
        analysisResult,
        message: 'Server auto-analyzed and actions triggered'
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
      ...
      await supabase.from('server_analysis_queue').insert({
        server_data: serverData,
        analysis_type: 'manual_review_required',
        priority: 'critical',
        auto_triggered: true,
        risk_assessment: preCheck
      });

      return new Response(JSON.stringify({
        success: false,
        requiresManualReview: true,
        riskAssessment: preCheck,
        message: 'Server flagged for manual review'
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
