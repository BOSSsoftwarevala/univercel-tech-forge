/**
 * REPORTS SECTION
 * Revenue Reports, Sales Reports, Financial Reports, Tax Reports
 */
...
import {
  ...
  Download, Calendar, BarChart3, Printer
} from 'lucide-react';
...
const configs: Record<string, { title: string; desc: string; icon: React.ElementType; color: string; metrics: { label: string; value: string }[] }> = {
  rpt_revenue_reports: {
    ...
    desc: 'Detailed revenue analysis and breakdowns',
    icon: TrendingUp,
    color: '#2ca01c',
    metrics: [
      { label: 'Total Revenue', value: '₹0.00' },
      { label: 'Subscription Revenue', value: '₹0.00' },
      { label: 'One-time Sales', value: '₹0.00' },
      { label: 'Commission Revenue', value: '₹0.00' },
    ],
  },
  rpt_sales_reports: {
    title: 'Sales Reports',
    desc: 'Sales performance and pipeline reports',
    icon: PieChart,
    color: '#3b82f6',
    metrics: [
  ...
  rpt_financial_reports: {
    ...
    desc: 'Comprehensive financial statements and summaries',
    icon: FileSpreadsheet,
    color: '#8b5cf6',
    metrics: [
      { label: 'Total Assets', value: '₹0.00' },
      { label: 'Total Liabilities', value: '₹0.00' },
      { label: 'Net Worth', value: '₹0.00' },
      { label: 'Cash on Hand', value: '₹0.00' },
    ],
  },
  rpt_tax_reports: {
    title: 'Tax Reports',
    desc: 'Tax compliance and filing reports',
    icon: FileCheck,
    color: '#f59e0b',
    metrics: [
      { label: 'GST Payable', value: '₹0.00' },
      { label: 'TDS Payable', value: '₹0.00' },
      { label: 'Input Tax Credit', value: '₹0.00' },
      { label: 'Net Tax Liability', value: '₹0.00' },
    ],
  },
};
        ...
        <CardContent className="p-6">
          <div className="h-[280px] flex items-center justify-center border-2 border-dashed border-[#e5e7eb] rounded-lg">
            <div className="text-center">
              ...
              <p className="text-[13px] text-[#9ca3af]">Report charts will render with real data</p>
              <p className="text-[11px] text-[#d1d5db] mt-1">Connect data sources to generate reports</p>
            </div>
          </div>
        </CardContent>
      </Card>
