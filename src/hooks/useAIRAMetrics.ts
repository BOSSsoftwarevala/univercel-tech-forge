  ...
  const fetchMetrics = useCallback(async () => {
    try {
      ...
      const [
        ...
        supabase.from('marketplace_orders').select('id, final_amount, created_at'),
        supabase.from('products').select('id, category', { count: 'exact' }).eq('is_active', true),
        supabase.from('server_instances').select('id', { count: 'exact' }).eq('status', 'running'),
        supabase.from('approvals').select('id', { count: 'exact' }).eq('status', 'pending'),
        supabase.from('system_alerts').select('id', { count: 'exact' }).eq('is_resolved', false),
        supabase.from('audit_logs').select('id, module, action, role, timestamp').order('timestamp', { ascending: false }).limit(200),
        supabase.from('activity_log').select('id, action_type, entity_type, role, severity_level, created_at').order('created_at', { ascending: false }).limit(100),
