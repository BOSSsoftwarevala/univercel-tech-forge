...
const OverviewView = () => {
  return (
    <div className="space-y-8">
      ...
      <div>
        <h2 className="text-xl font-semibold text-stone-800 mb-5">Global Summary</h2>
        <div className="grid grid-cols-4 gap-4">
          {summaryCards.map((card) => (
            <div 
              key={card.label} 
              className="p-5 bg-white rounded-xl shadow-sm border border-stone-100"
            >
              <div className="flex items-start justify-between">
                ...
                <div className="w-10 h-10 bg-stone-50 rounded-lg flex items-center justify-center">
                  <card.icon className="w-5 h-5 text-stone-400" />
                </div>
              </div>
            </div>
          ))}
      ...
      <div>
        <h2 className="text-xl font-semibold text-stone-800 mb-5">Continent Snapshot</h2>
        <div className="bg-white rounded-xl shadow-sm border border-stone-100 overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-stone-100">
                <th className="text-left px-6 py-4 text-sm font-medium text-stone-500">Continent Name</th>
                <th className="text-left px-6 py-4 text-sm font-medium text-stone-500">Super Admin</th>
                <th className="text-left px-6 py-4 text-sm font-medium text-stone-500">Active Countries</th>
                <th className="text-left px-6 py-4 text-sm font-medium text-stone-500">Revenue Trend</th>
              </tr>
            </thead>
            <tbody>
              {continentData.map((row, idx) => (
                ...
                >
                  <td className="px-6 py-4 font-medium text-stone-800">{row.name}</td>
                  <td className="px-6 py-4 text-stone-600">{row.admin}</td>
                  <td className="px-6 py-4 text-stone-600">{row.countries}</td>
